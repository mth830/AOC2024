...
let arena = new Array(ARENA_HEIGHT).fill().map(row => new Array(ARENA_WIDTH).fill('.'));
...
for (const robot of robots) {
  let [[row, col], [rowVelocity, colVelocity]] = robot;
  let latestRow = mod((row + rowVelocity * STEPS), ARENA_HEIGHT);
  let latestCol = mod((col + colVelocity * STEPS), ARENA_WIDTH);
...
let result = quadrants.reduce((prod, row) => prod * row.reduce((prod1, val) => prod1 * val, 1), 1)
console.log(result)
