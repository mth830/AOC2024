...
let minPushes = ([a, b, prize]) => {
  ...
  if (aPresses % 1 !== 0 || bPresses % 1 !== 0) return Infinity;
  if ((aPresses * ax + bPresses * bx) === px && (aPresses * ay + bPresses * by === py)) return (3 * aPresses + bPresses)
}
const output = map.map(minPushes).filter(x => x !== Infinity).reduce((sum, x) => sum + x, 0);
console.log(output)
